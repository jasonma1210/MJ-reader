...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // PATCH(mjnexus): must autorelease the returned config. ARC callers expect a
      // +0 autoreleased return; returning the raw +1 pointer made the Retained drop
      // dealloc the object before UIKit retained it -> SIGSEGV in objc_retain
      // inside -[UIApplication _connectUISceneFromFBSScene:] (iOS 27 SDK).
